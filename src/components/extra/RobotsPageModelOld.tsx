...
import { RobotModal } from '@/components/RobotShowcaseSection'
...
function RobotCard({
  ...
  return (
    ...
    >
      ...
      {/* ── Card body ── */}
      <div style={{ padding:'1.25rem 1.25rem 1.5rem', position:'relative', zIndex:2, transformStyle:'preserve-3d' }}>
...
export default function RobotsPage() {
  ...
  return (
    <>
      {modalRobot && <RobotModal robot={modalRobot} onClose={() => setModalRobot(null)} />}
          ...
          {filtered.length > 0 ? (
            <div className="robots-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'1.5rem', perspective:'1200px' }}>
              {filtered.map((robot, i) => (
                <RobotCard key={robot.id} robot={robot} index={i} visible={visible} onOpen={() => setModalRobot(robot)}/>
              ))}
            </div>
          ) : (
