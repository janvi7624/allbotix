  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i=0;i<pts.length;i++) for (let j=i+1;j<pts.length;j++) {
        ...
        if(d<100){ctx.beginPath();ctx.strokeStyle=`rgba(176,58,46,${0.1*(1-d/100)})`;ctx.lineWidth=0.5;ctx.moveTo(pts[i].x,pts[i].y);ctx.lineTo(pts[j].x,pts[j].y);ctx.stroke()}
      }
...
function TeamCard({ member, visible }: { member: typeof TEAM[0]; visible: boolean }) {
  ...
  const onMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (shineRef.current) {
      ...
      shineRef.current.style.background = `radial-gradient(circle at ${px}% ${py}%, rgba(232,57,42,0.15) 0%, transparent 62%)`
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          border: `1px solid ${hovered && !flipped ? 'rgba(176,58,46,0.55)' : 'rgba(176,58,46,0.18)'}`,
          background: 'var(--bg-card)',
          boxShadow: hovered && !flipped
            ? '0 28px 70px rgba(0,0,0,0.65), 0 0 36px rgba(176,58,46,0.14)'
          ...
          <div aria-hidden="true" style={{ position:'absolute',inset:0,overflow:'hidden',zIndex:2,pointerEvents:'none' }}>
            <div style={{ position:'absolute',left:0,right:0,height:'3px',background:'linear-gradient(90deg,transparent,rgba(176,58,46,0.35),transparent)',animation:`scanline 4s linear ${member.index*0.8}s infinite` }}/>
          </div>
          ...
          <div style={{ position:'relative',height:'260px',overflow:'hidden' }}>
            ...
            {/* Dept tag */}
            <div style={{ position:'absolute',top:'12px',left:'12px',padding:'4px 12px',borderRadius:'100px',background:'rgba(8,8,8,0.8)',border:'1px solid rgba(176,58,46,0.4)',backdropFilter:'blur(8px)',zIndex:2 }}>
            ...
            {/* Flip hint */}
            <div style={{ position:'absolute',top:'12px',right:'12px',padding:'4px 10px',borderRadius:'100px',background:'rgba(176,58,46,0.15)',border:'1px solid rgba(176,58,46,0.35)',zIndex:2,opacity: hovered && !flipped ? 1 : 0,transition:'opacity 0.3s' }}>
          ...
          <div style={{ padding:'1.5rem 1.5rem 1.25rem' }}>
            <div style={{ display:'flex',alignItems:'center',gap:'8px',marginBottom:'6px' }}>
              <span style={{ width:'5px',height:'5px',borderRadius:'50%',background:'var(--red-bright)',flexShrink:0,boxShadow:'0 0 8px rgba(176,58,46,0.7)',animation:'dotPulse 2s ease-in-out infinite' }}/>
            ...
            <h3 style={{ fontFamily:'var(--font-display)',fontSize:'1.2rem',fontWeight:900,color:'var(--text-primary)',letterSpacing:'0.03em',marginBottom:'4px',lineHeight:1.1,
        ...
        <div style={{
          ...
          background:'linear-gradient(145deg,rgba(22,10,10,0.98),rgba(12,8,8,0.99))',
          boxShadow:'0 28px 70px rgba(0,0,0,0.7), 0 0 40px rgba(176,58,46,0.16)',
          display:'flex',flexDirection:'column',padding:'2rem 1.75rem',
          gap:'1.25rem',
        }}>
          {/* Corner glow */}
          <div aria-hidden="true" style={{ position:'absolute',top:'-1px',right:'-1px',width:'120px',height:'120px',background:'radial-gradient(circle at top right,rgba(176,58,46,0.22) 0%,transparent 70%)',borderRadius:'0 14px 0 0',pointerEvents:'none' }}/>
          <div aria-hidden="true" style={{ position:'absolute',bottom:'-1px',left:'-1px',width:'100px',height:'100px',background:'radial-gradient(circle at bottom left,rgba(176,58,46,0.12) 0%,transparent 70%)',borderRadius:'0 0 0 14px',pointerEvents:'none' }}/>
          ...
          <div>
            ...
            <div style={{ display:'flex',flexWrap:'wrap',gap:'6px' }}>
              {member.skills.map(s => (
                <span key={s} style={{ padding:'4px 10px',borderRadius:'100px',border:'1px solid rgba(176,58,46,0.3)',background:'rgba(176,58,46,0.06)',fontFamily:'var(--font-display)',fontSize:'0.52rem',letterSpacing:'0.1em',textTransform:'uppercase',color:'rgba(176,58,46,0.85)' }}>{s}</span>
              ))}
            </div>
          </div>

          <div style={{ marginTop:'auto',display:'flex',gap:'10px' }}>
            <a href={`mailto:${member.social.email}`} style={{ flex:1,padding:'10px',background:'linear-gradient(135deg,rgba(176,58,46,0.2),rgba(176,58,46,0.08))',border:'1px solid rgba(176,58,46,0.4)',borderRadius:'8px',textAlign:'center',fontFamily:'var(--font-display)',fontSize:'0.55rem',letterSpacing:'0.14em',textTransform:'uppercase',color:'var(--red-bright)',textDecoration:'none',transition:'all 0.25s',display:'flex',alignItems:'center',justifyContent:'center',gap:'6px' }}
              onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background='rgba(176,58,46,0.25)';(e.currentTarget as HTMLElement).style.boxShadow='0 0 20px rgba(176,58,46,0.3)'}}
              onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background='linear-gradient(135deg,rgba(176,58,46,0.2),rgba(176,58,46,0.08))';(e.currentTarget as HTMLElement).style.boxShadow='none'}}
            >
            ...
            <a href={member.social.linkedin} style={{ flex:1,padding:'10px',background:'rgba(176,58,46,0.04)',border:'1px solid rgba(176,58,46,0.25)',borderRadius:'8px',textAlign:'center',fontFamily:'var(--font-display)',fontSize:'0.55rem',letterSpacing:'0.14em',textTransform:'uppercase',color:'var(--text-muted)',textDecoration:'none',transition:'all 0.25s',display:'flex',alignItems:'center',justifyContent:'center',gap:'6px' }}
              onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.borderColor='rgba(176,58,46,0.5)';(e.currentTarget as HTMLElement).style.color='var(--red-bright)'}}
              onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor='rgba(176,58,46,0.25)';(e.currentTarget as HTMLElement).style.color='var(--text-muted)'}}
            >
            ...
            <button onClick={e=>{e.stopPropagation();setFlipped(false)}} style={{ width:'40px',flexShrink:0,background:'rgba(176,58,46,0.04)',border:'1px solid rgba(176,58,46,0.2)',borderRadius:'8px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'var(--text-muted)',transition:'all 0.25s' }}
              onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.borderColor='rgba(176,58,46,0.5)';(e.currentTarget as HTMLElement).style.color='var(--red-bright)'}}
              onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor='rgba(176,58,46,0.2)';(e.currentTarget as HTMLElement).style.color='var(--text-muted)'}}
            >
...
function ValueCard({ item, index, visible }: { item:typeof VALUES[0]; index:number; visible:boolean }) {
  ...
  const onMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (shineRef.current) {
      ...
      shineRef.current.style.background=`radial-gradient(circle at ${px}% ${py}%,rgba(232,57,42,0.12) 0%,transparent 58%)`
      shineRef.current.style.opacity='1'
    }
  }
  const onLeave = () => {
  ...
  return (
    <div ref={cardRef} onMouseMove={onMove} onMouseEnter={()=>setHovered(true)} onMouseLeave={onLeave}
      style={{
        ...
        boxShadow: hovered?'0 24px 60px rgba(0,0,0,0.6),0 0 28px rgba(176,58,46,0.12)':'none',
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          filter: hovered ? 'drop-shadow(0 0 10px rgba(232,57,42,0.45))' : 'none',
        }}
...
export default function TeamSection() {
  ...
  return (
    <div style={{ background:'var(--bg-900)', minHeight:'100vh' }}>
      ...
      {/* ══ HERO ══════════════════════════════════════════════════════════════ */}
      <section style={{ position:'relative', minHeight:'60vh', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', paddingBlock:'8rem 6rem', background:`radial-gradient(ellipse 80% 60% at 50% 40%,rgba(176,58,46,0.1) 0%,transparent 70%),var(--bg-900)` }}>
        <ParticleField/>

        {/* Rings */}
        <div aria-hidden="true" style={{ position:'absolute',width:'600px',height:'600px',borderRadius:'50%',border:'1px dashed rgba(176,58,46,0.1)',top:'50%',left:'50%',animation:'ringCW 30s linear infinite',pointerEvents:'none' }}/>
        <div aria-hidden="true" style={{ position:'absolute',width:'400px',height:'400px',borderRadius:'50%',border:'1px solid rgba(176,58,46,0.13)',top:'50%',left:'50%',animation:'ringCCW 20s linear infinite',pointerEvents:'none' }}/>
        ...
        <div ref={heroRef} className="container-allbotix" style={{ position:'relative',zIndex:2,textAlign:'center' }}>
          ...
          <h1 style={{
            ...
            Meet the <span style={{ color:'var(--red-bright)', textShadow:'0 0 50px rgba(176,58,46,0.4)' }}>Minds</span><br/>
            Shaping Tomorrow.
          </h1>
      ...
      {/* ══ STATS BAR ════════════════════════════════════════════════════════ */}
      <section ref={statsRef} style={{ position:'relative',borderTop:'1px solid rgba(176,58,46,0.15)',borderBottom:'1px solid rgba(176,58,46,0.15)',background:'rgba(176,58,46,0.03)',paddingBlock:'3rem',overflow:'hidden' }}>
        <div aria-hidden="true" style={{ position:'absolute',inset:0,background:'repeating-linear-gradient(45deg,rgba(176,58,46,0.012) 0px,rgba(176,58,46,0.012) 1px,transparent 1px,transparent 22px)',pointerEvents:'none' }}/>
        <div className="container-allbotix" style={{ position:'relative',zIndex:2 }}>
          <div className="stats-row" style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'1.5px',background:'rgba(176,58,46,0.15)',border:'1px solid rgba(176,58,46,0.15)',borderRadius:'8px',overflow:'hidden' }}>
            {[
              ...
              <div key={s.label} style={{ background:'rgba(11,11,11,0.95)',padding:'2rem 1.5rem',opacity:statsStarted?1:0,transform:statsStarted?'translateY(0)':'translateY(20px)',transition:`opacity 0.6s ease ${i*0.12+0.1}s,transform 0.6s ease ${i*0.12+0.1}s` }}>
                <StatCounter target={s.target} suffix={s.suffix} label={s.label} started={statsStarted} delay={i*180}/>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="team" ref={teamRef} style={{ position:'relative',paddingBlock:'6rem',background:'var(--bg-800)',overflow:'hidden',borderTop:'1px solid var(--border-soft)' }}>
        <div aria-hidden="true" style={{ position:'absolute',top:'15%',right:'-8%',width:'500px',height:'500px',borderRadius:'50%',background:'radial-gradient(circle,rgba(176,58,46,0.07) 0%,transparent 70%)',pointerEvents:'none',animation:'glowFloat 6s ease-in-out infinite' }}/>
        <div aria-hidden="true" style={{ position:'absolute',bottom:'15%',left:'-8%',width:'380px',height:'380px',borderRadius:'50%',background:'radial-gradient(circle,rgba(176,58,46,0.05) 0%,transparent 70%)',pointerEvents:'none',animation:'glowFloat 8s ease-in-out 2s infinite' }}/>
        {/* Watermark */}
        ...
        <div className="container-allbotix" style={{ position:'relative',zIndex:2 }}>
          <div style={{ textAlign:'center',marginBottom:'4rem',opacity:teamVisible?1:0,transform:teamVisible?'translateY(0)':'translateY(24px)',filter:teamVisible?'none':'blur(3px)',transition:'opacity 0.7s ease,transform 0.7s ease,filter 0.7s ease' }}>
            ...
            {/* Flip hint badge */}
            <div style={{ display:'inline-flex',alignItems:'center',gap:'8px',marginTop:'1rem',padding:'6px 16px',borderRadius:'100px',background:'rgba(176,58,46,0.08)',border:'1px solid rgba(176,58,46,0.3)' }}>
      ...
      <section style={{ position:'relative',paddingBlock:'5rem',background:'var(--bg-800)',overflow:'hidden',borderTop:'1px solid var(--border-soft)',textAlign:'center' }}>
        <ParticleField/>
        <div aria-hidden="true" style={{ position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',width:'600px',height:'300px',borderRadius:'50%',background:'radial-gradient(ellipse,rgba(176,58,46,0.09) 0%,transparent 70%)',pointerEvents:'none',animation:'glowFloat 5s ease-in-out infinite' }}/>
        <div className="container-allbotix" style={{ position:'relative',zIndex:2 }}>
