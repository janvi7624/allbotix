...
const MODAL_STYLES = `
  ...
  .robo-close-btn {
    ...
    border:1px solid rgba(176,58,46,0.3);background:rgba(176,58,46,0.08);
    display:flex;align-items:center;justify-content:center;
    cursor:pointer;transition:all 0.25s;flex-shrink:0;
  }
  .robo-close-btn:hover { background:rgba(176,58,46,0.22);border-color:var(--red-bright);transform:rotate(90deg);box-shadow:0 0 18px rgba(176,58,46,0.35); }

  .robo-nav-btn {
    width:40px;height:40px;border-radius:50%;
    border:1px solid rgba(176,58,46,0.25);background:rgba(176,58,46,0.06);
    display:flex;align-items:center;justify-content:center;
    cursor:pointer;transition:all 0.25s;flex-shrink:0;
  }
  .robo-nav-btn:hover { background:rgba(176,58,46,0.18);border-color:var(--red-bright);box-shadow:0 0 14px rgba(176,58,46,0.3); }

  .robo-thumb {
    cursor:pointer;border-radius:8px;overflow:hidden;
    border:1px solid rgba(176,58,46,0.15);transition:all 0.28s;flex-shrink:0;
  }
  .robo-thumb:hover  { border-color:rgba(176,58,46,0.55);transform:translateY(-2px); }
  .robo-thumb.active { border-color:var(--red-bright);box-shadow:0 0 14px rgba(176,58,46,0.4); }

  /* sections */
  .robo-detail-section { padding:2rem 0;border-top:1px solid rgba(176,58,46,0.1); }
  ...
  .robo-img-block {
    ...
    border:1px solid rgba(176,58,46,0.2);background:rgba(176,58,46,0.03);aspect-ratio:4/3;
  }
  ...
  .robo-model-card:hover { border-color:rgba(176,58,46,0.5);box-shadow:0 0 20px rgba(176,58,46,0.12);transform:translateY(-3px); }
  ...
  .robo-pdf-btn:hover { background:rgba(176,58,46,0.2);box-shadow:0 0 14px rgba(176,58,46,0.25); }

  /* use-case accordion */
  .robo-usecase-row {
    border:1px solid rgba(176,58,46,0.15);border-radius:10px;overflow:hidden;
    margin-bottom:8px;transition:border-color 0.22s;
  }
  .robo-usecase-row.open { border-color:rgba(176,58,46,0.45); }
  .robo-usecase-header {
    display:flex;align-items:center;justify-content:space-between;gap:12px;
    padding:12px 16px;cursor:pointer;background:rgba(176,58,46,0.04);transition:background 0.2s;
  }
  .robo-usecase-header:hover { background:rgba(176,58,46,0.08); }
  ...
  /* packaging */
  .robo-pkg-item { padding:10px 14px;border-radius:8px;border:1px solid rgba(176,58,46,0.15);background:rgba(176,58,46,0.04);animation:robo-slideIn 0.4s ease both; }
  ...
  .robo-spec-pill {
    ...
    border:1px solid rgba(176,58,46,0.18);background:rgba(176,58,46,0.05);
  ...
  .robo-cta-btn {
    ...
    background:linear-gradient(135deg,rgba(176,58,46,0.2),rgba(176,58,46,0.08));
  ...
  .robo-cta-btn:hover { background:rgba(176,58,46,0.28);box-shadow:0 0 20px rgba(176,58,46,0.3); }
        ...
        {models.map((m, i) => (
          <div key={m.id} className="robo-model-card" style={{ animationDelay: `${i * 0.07}s` }}>
            ...
            <div style={{ position: 'relative', height: '120px', overflow: 'hidden' }}>
              ...
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top,rgba(4,4,4,0.75) 0%,transparent 55%)' }} />
              <p style={{ position: 'absolute', bottom: '8px', left: '10px', right: '10px', fontFamily: 'var(--font-display)', fontSize: '0.72rem', fontWeight: 700, color: '#fff', letterSpacing: '0.06em' }}>{m.name}</p>
            </div>
            {/* body */}
    ...
    <div className="robo-detail-section" style={{ animation: `robo-fadeUp 0.5s ease ${delay}s both` }}>
      <div className="robo-grid-2">
        ...
        <div className="robo-img-block">
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top,rgba(4,4,4,0.5) 0%,transparent 55%)' }} />
          <div style={{ position: 'absolute', bottom: '10px', left: '12px', padding: '4px 12px', borderRadius: '100px', background: 'rgba(4,4,4,0.78)', border: '1px solid rgba(176,58,46,0.35)', backdropFilter: 'blur(6px)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ height: '1px', background: 'linear-gradient(90deg,rgba(176,58,46,0.35),transparent)' }} />
          ...
          <div key={uc.id} className={`robo-usecase-row${isOpen ? ' open' : ''}`}>
            ...
            <div className="robo-usecase-header" onClick={() => setOpenIdx(isOpen ? null : i)}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
                <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: isOpen ? 'var(--red-bright)' : 'rgba(176,58,46,0.35)', flexShrink: 0, transition: 'background 0.22s' }} />
              ...
              <svg className={`robo-usecase-chevron${isOpen ? ' open' : ''}`} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(176,58,46,0.7)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {isPackaging && section.includes && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              {section.ctaLink && (
                <a href={section.ctaLink} target="_blank" rel="noopener noreferrer" className="robo-pdf-btn" style={{ marginTop: '4px', width: 'fit-content' }}>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '65fr 35fr', gap: '2rem', padding: '2rem 2.5rem 0', alignItems: 'center' }} className="robo-grid-2">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', animation: 'robo-fadeUp 0.5s ease 0.1s both' }}>
          {/* badges */}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            <span style={{ padding: '4px 12px', borderRadius: '100px', background: 'rgba(176,58,46,0.1)', border: '1px solid rgba(176,58,46,0.3)', fontFamily: 'var(--font-display)', fontSize: '0.5rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--red-bright)' }}>
              {robot.category}
            </span>
            <span style={{ padding: '4px 12px', borderRadius: '100px', background: 'rgba(176,58,46,0.05)', border: '1px solid rgba(176,58,46,0.18)', fontFamily: 'var(--font-display)', fontSize: '0.5rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>
              {robot.tag}
            </span>
            {r.modelNumber && (
              <span style={{ padding: '4px 12px', borderRadius: '100px', background: 'transparent', border: '1px solid rgba(176,58,46,0.15)', fontFamily: 'var(--font-display)', fontSize: '0.5rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>
                {r.modelNumber}
              </span>
            )}
          </div>
          ...
          {hasModels && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ padding: '5px 14px', borderRadius: '100px', background: 'rgba(176,58,46,0.1)', border: '1px solid rgba(176,58,46,0.3)', fontFamily: 'var(--font-display)', fontSize: '0.56rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--red-bright)' }}>
                {r.models.length} Models Available ↓
              </span>
            </div>
          )}

          <button className="robo-cta-btn" style={{ width: 'fit-content', marginTop: '4px' }}>
            Request Deployment →
          </button>
        </div>
        {/* floating image */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', minHeight: '300px' }}>
          <div aria-hidden="true" style={{ position: 'absolute', top: '50%', left: '50%', width: '240px', height: '240px', borderRadius: '50%', border: '1px solid rgba(176,58,46,0.07)', transform: 'translate(-50%,-50%)', animation: 'robo-ringCW 18s linear infinite', pointerEvents: 'none' }} />
          <div aria-hidden="true" style={{ position: 'absolute', top: '50%', left: '50%', width: '340px', height: '340px', borderRadius: '50%', border: '1px solid rgba(176,58,46,0.04)', transform: 'translate(-50%,-50%)', animation: 'robo-ringCCW 26s linear infinite', pointerEvents: 'none' }} />
          <div aria-hidden="true" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: '380px', height: '380px', borderRadius: '50%', background: 'radial-gradient(circle,rgba(176,58,46,0.09) 0%,transparent 65%)', pointerEvents: 'none' }} />
          <div style={{ animation: 'robo-floatUD 4s ease-in-out infinite', position: 'relative', zIndex: 2 }}>
            <div aria-hidden="true" style={{ position: 'absolute', inset: 0, overflow: 'hidden', borderRadius: '18px', zIndex: 3, pointerEvents: 'none' }}>
              <div style={{ position: 'absolute', left: 0, right: 0, height: '3px', background: 'linear-gradient(90deg,transparent,rgba(176,58,46,0.4),transparent)', animation: 'robo-scanModal 3.5s linear infinite' }} />
            </div>
            <div style={{ width: '400px', height: '290px', borderRadius: '18px', overflow: 'hidden', border: '1px solid rgba(176,58,46,0.4)', boxShadow: '0 0 50px rgba(176,58,46,0.15),0 30px 70px rgba(0,0,0,0.55)', position: 'relative' }}>
      ...
      <div style={{ padding: '0 2.5rem 2.5rem' }}>
        ...
        {!populatedSections.length && !hasModels && !hasUseCases && (
          <div style={{ padding: '2rem 0', borderTop: '1px solid rgba(176,58,46,0.1)', marginTop: '2rem' }}>
...
export function RobotModal({ robot, onClose }: { robot: RobotWithIndex; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 999, background: 'rgba(4,4,4,0.97)', backdropFilter: 'blur(18px)', display: 'flex', flexDirection: 'column', opacity: phase === 'exit' ? 0 : 1, transition: 'opacity 0.45s ease', padding: '0 1rem' }}>
      <style>{MODAL_STYLES}</style>

      {/* top bar */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.9rem 1.75rem', borderBottom: '1px solid rgba(176,58,46,0.12)', flexShrink: 0, gap: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: 'var(--red-bright)', animation: 'robo-pulse 2s ease-in-out infinite', boxShadow: '0 0 8px rgba(176,58,46,0.8)', flexShrink: 0, display: 'inline-block' }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, justifyContent: 'center', overflow: 'hidden' }}>
          <button className="robo-nav-btn" onClick={() => setCarIdx(p => mod(p - 1, N))} aria-label="Previous">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.7)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
          </button>
          ...
          <button className="robo-nav-btn" onClick={() => setCarIdx(p => mod(p + 1, N))} aria-label="Next">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.7)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6" /></svg>
          </button>
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
        ...
        {/* fleet strip */}
        <div style={{ width: 'min(175px,20vw)', flexShrink: 0, borderLeft: '1px solid rgba(176,58,46,0.1)', padding: '1.25rem 0.875rem', display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', opacity: thumbVisible ? 1 : 0, transform: thumbVisible ? 'translateX(0)' : 'translateX(40px)', transition: 'opacity 0.5s ease,transform 0.5s ease' }}>
          ...
          {ROBOTS.map((r, i) => (
            <div key={r.name} className={`robo-thumb${i === carIdx ? ' active' : ''}`} onClick={() => setCarIdx(i)} role="button" aria-label={r.name}>
              <div style={{ position: 'relative', height: '120px' }}>
                ...
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top,rgba(4,4,4,0.88),transparent 55%)' }} />
                <p style={{ position: 'absolute', bottom: '6px', left: '8px', right: '4px', fontFamily: 'var(--font-display)', fontSize: '0.52rem', fontWeight: 700, color: 'rgba(255,255,255,0.9)', letterSpacing: '0.05em', lineHeight: 1.2 }}>{r.name}</p>
      ...
      {/* bottom dots */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '0.8rem 1rem', borderTop: '1px solid rgba(176,58,46,0.08)', flexShrink: 0 }}>
        {ROBOTS.map((_, i) => (
          <div key={i} onClick={() => setCarIdx(i)} role="button"
            style={{ width: i === carIdx ? '22px' : '6px', height: '6px', borderRadius: i === carIdx ? '3px' : '50%', background: i === carIdx ? 'var(--red-bright)' : 'rgba(176,58,46,0.3)', cursor: 'pointer', transition: 'all 0.3s ease', boxShadow: i === carIdx ? '0 0 8px rgba(176,58,46,0.6)' : 'none' }}
          />
        ))}
      </div>
    </div>
    ...
    <>
      ...
      <section id="project" style={{ position: 'relative', paddingBlock: '6rem', background: 'var(--bg-900)', overflow: 'hidden', borderTop: '1px solid var(--border-soft)' }}>
        <style>{`
          ...
          .rc-dot { width:6px;height:6px;border-radius:50%;border:1px solid rgba(176,58,46,0.4);background:transparent;cursor:pointer;transition:all 0.25s; }
          ...
          .rc-robo-nav-btn { display:flex;align-items:center;gap:7px;padding:6px 14px;border-radius:100px;border:1px solid rgba(176,58,46,0.2);background:rgba(176,58,46,0.05);cursor:pointer;transition:all 0.25s;flex-shrink:0;font-family:var(--font-display);font-size:0.52rem;letter-spacing:0.14em;text-transform:uppercase;color:var(--text-muted); }
          .rc-robo-nav-btn:hover,.rc-robo-nav-btn.active { border-color:rgba(176,58,46,0.55);background:rgba(176,58,46,0.12);color:var(--red-bright); }
          .rc-robo-nav-btn .dot { width:5px;height:5px;border-radius:50%;background:rgba(176,58,46,0.3);flex-shrink:0;transition:background 0.25s; }
        ...
        <div className="container-allbotix" style={{ position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '2.5rem' }}>
            ...
            <button className="rc-robo-nav-btn" onClick={() => openModal(current)} style={{ marginLeft: 'auto', border: '1px solid rgba(176,58,46,0.45)', background: 'rgba(176,58,46,0.1)', color: 'var(--red-bright)' }}>
            ...
            {ROBOTS.map((r, i) => {
              ...
              return (
                <div key={r.name} onClick={() => { if (isDrag.current) return; if (isCenter) openModal(i); else { goTo(i); resetTimer() } }} style={{ position: 'absolute', ...cardStyle(i), transformOrigin: slot >= 0 ? 'left center' : 'right center' }}>
                  <div className="rc-card-inner" style={{ width: '200px', height: isCenter ? '300px' : '260px', borderRadius: '14px', overflow: 'hidden', border: isCenter ? '1px solid rgba(176,58,46,0.45)' : '1px solid rgba(176,58,46,0.15)', background: 'var(--bg-card)', position: 'relative', cursor: isCenter ? 'pointer' : 'default' }}>
                    {isCenter && <div aria-hidden="true" style={{ position: 'absolute', left: 0, right: 0, height: '3px', background: 'linear-gradient(90deg,transparent,rgba(176,58,46,0.35),transparent)', animation: 'rc-scanline 3s linear infinite', pointerEvents: 'none', zIndex: 3 }} />}
                    <Image src={r.src} alt={r.name} loading="lazy" width={200} height={300} style={{ objectFit: 'cover' }} />
                    {!isCenter && <div style={{ position: 'absolute', bottom: '10px', left: '50%', transform: 'translateX(-50%)', fontFamily: 'var(--font-display)', fontSize: '0.55rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)', background: 'rgba(8,8,8,0.6)', padding: '4px 10px', borderRadius: '4px', backdropFilter: 'blur(6px)', whiteSpace: 'nowrap' }}>{r.name}</div>}
                    {isCenter && (
                      <>
                        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '1.25rem 1rem 1rem', background: 'linear-gradient(to top,rgba(8,8,8,0.92) 0%,transparent 100%)', zIndex: 4 }}>
                        ...
                        <div style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 5, padding: '3px 9px', borderRadius: '100px', background: 'rgba(176,58,46,0.2)', border: '1px solid rgba(176,58,46,0.4)', fontFamily: 'var(--font-display)', fontSize: '0.45rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--red-bright)', backdropFilter: 'blur(6px)' }}>TAP TO EXPAND</div>
                        <div aria-hidden="true" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: '180px', height: '180px', borderRadius: '50%', border: '1px solid rgba(176,58,46,0.08)', pointerEvents: 'none', zIndex: 2, animation: 'rc-ringCW 12s linear infinite' }} />
                      </>
                    )}
                  </div>
                </div>
          ...
          <div key={current} className="rc-info-enter" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1.5rem', marginTop: '2.5rem', flexWrap: 'wrap' }}>
            <div style={{ flex: '1', minWidth: '240px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '0.75rem' }}>
                {robot.specs.map(spec => (
                  <span key={spec} style={{ padding: '4px 10px', borderRadius: '100px', border: '1px solid rgba(176,58,46,0.2)', background: 'rgba(176,58,46,0.05)', fontFamily: 'var(--font-display)', fontSize: '0.58rem', color: 'var(--text-muted)', letterSpacing: '0.04em' }}>{spec}</span>
                ))}
              </div>
            </div>
